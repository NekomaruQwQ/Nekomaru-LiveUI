...
//
// Ring buffer capacity: ~100ms at 48kHz stereo = 4800 frames × 2 channels.
// WebSocket push eliminates HTTP polling jitter, so we can use a smaller
// buffer than the old HTTP-polling architecture (which needed 200ms).
//
// Pre-buffering: the worklet outputs silence until the ring has accumulated
// at least PRE_BUFFER_FRAMES of audio (~50ms).  This is a one-shot gate —
...
const RING_CAPACITY_FRAMES = 4800;
